...
function onCreatureSay(cid, type, msg)
  	...
  	if (msgcontains(msg, 'hi') and (focus == 0)) and getDistanceToCreature(cid) < 4 then
  		selfSay('Hello, ' .. creatureGetName(cid) .. '! Eu do golden blade para pega a sua basta falar GOLDEN BLADE.')
  		focus = cid
  		talk_start = os.clock()
	...
	elseif focus == cid then
		...
		if msgcontains(msg, 'golden blade') then
			buy(cid,7417,getCount(msg),0)
