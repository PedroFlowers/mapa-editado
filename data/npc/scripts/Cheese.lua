...
function onCreatureSay(cid, type, msg)
    ...
    if (msgcontains(msg, 'hi') and (focus == 0)) and getDistanceToCreature(cid) < 4 then
        selfSay('Olá ' .. getCreatureName(cid) .. '... I Change Item For Crystal')
        focus = cid
        talk_start = os.clock()
    ...
    elseif focus == cid then
        talk_start = os.clock()
        if msgcontains(msg, 'amulet') or msgcontains(msg, 'change item') then
            if doPlayerRemoveItem(cid,6527,1) == 0 then
